...
x_train[(np.abs(stats.zscore(x_train)) < 2).all(axis=1)]
q = x_train["col"].quantile(0.99)
x_train[x_train["col"] < q]
...
y_test = scPrice.transform(y_test.reshape(-1,1))
...
classifier.fit(x_train, y_train, batch_size = 64, epochs =25, verbose=1, validation_split=0.2) # Remove validation speed before upload to kaggle
